 ...
 */
const findTaskMakerExecutable = async (): Promise<string> => {
 ...
 */
export const taskMetadata = async (path: string): Promise<TaskMetadata> => {
    const args = ['--ui=json', '--task-info', '--task-dir', path];
    let metadata = '';
    const process = spawn(await findTaskMakerExecutable(), args);
...
taskMetadata('/home/ale/git/oii/oii/problemi/taglialegna')
    .then((metadata) => {
        console.log(metadata);
    })
    .catch((error) => {
